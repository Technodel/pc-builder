...
# --- 1. THE RESET ENGINE (The part that worked!) ---
if 'rid' not in st.session_state:
    st.session_state.rid = 0

def hard_reset():
    st.session_state.rid += 1
    # We clear selections but keep the counts for RAM/Storage slots
    curr_rid = st.session_state.rid
    r_cnt = st.session_state.get('r_cnt', 1)
    s_cnt = st.session_state.get('s_cnt', 1)
    st.session_state.clear()
    st.session_state.rid = curr_rid
    st.session_state.r_cnt = r_cnt
    st.session_state.s_cnt = s_cnt

# --- 2. CONFIG & PREMIUM STYLING ---
...
if not raw_df.empty:
    rid = st.session_state.rid # Current reset version
    ...
    with st.sidebar:
        ...
        # RESET BUTTON (The hard reset logic)
    ...
    with col1:
        ...
        # Compatibility Filter applied here
        mb_list = mb_df[mb_df['ITEM'].apply(lambda x: is_compat(cpu_c, x))] if "Select" not in cpu_c else mb_df
        mb_c = st.selectbox("Motherboard", ["Select"] + [f"{r['ITEM']} - ${r['PRICE']}" for _,r in mb_list.iterrows()], key=f"m_{rid}")
        ...
        if "Select" not in mb_c:
            ...
            for i in range(st.session_state.r_cnt):
                st.selectbox(f"{tech} RAM {i+1}", ["Select"] + [f"{r['ITEM']} - ${r['PRICE']}" for _,r in ram_df.iterrows()], key=f"r_{i}_{rid}")
    ...
    with col2:
        ...
        psu_c = st.selectbox("PSU", ["Select"] + [f"{r['ITEM']} - ${r['PRICE']}" for _,r in psu_df.iterrows()], key=f"p_{rid}")
        cas_c = st.selectbox("Case", ["Select"] + [f"{r['ITEM']} - ${r['PRICE']}" for _,r in cas_df.iterrows()], key=f"ca_{rid}")
        coo_c = st.selectbox("Cooler", ["Select"] + [f"{r['ITEM']} - ${r['PRICE']}" for _,r in coo_df.iterrows()], key=f"co_{rid}")
    ...
    # --- 6. SUMMARY & EXPORT (TXT ONLY) ---
    st.divider()
    total = 0
    html_rows = ""
    txt_content = "TECHNODEL PC QUOTATION\n" + "="*25 + "\n"
    
    # Gather all parts
    part_keys = [(f'c_{rid}', 'CPU'), (f'm_{rid}', 'Motherboard'), (f'g_{rid}', 'GPU'), 
                 (f'p_{rid}', 'PSU'), (f'ca_{rid}', 'Case'), (f'co_{rid}', 'Cooler')]
    
    for k, label in part_keys:
        ...
        if val and "Select" not in val:
            ...
            txt_content += f"{label}: {val}\n"

    for i in range(st.session_state.get('r_cnt', 1)):
        v = st.session_state.get(f"r_{i}_{rid}")
        if v and "Select" not in v:
            name, price = v.split(" - $")
            total += int(price.replace(",", "")); txt_content += f"RAM: {v}\n"
            html_rows += f'<div class="build-item"><span>RAM {i+1}: {name}</span><b>${price}</b></div>'
            
    for i in range(st.session_state.get('s_cnt', 1)):
        v = st.session_state.get(f"s_{i}_{rid}")
        if v and "Select" not in v:
            name, price = v.split(" - $")
            total += int(price.replace(",", "")); txt_content += f"Storage: {v}\n"
    ...
    if total > 0:
        ...
        c_txt, c_wa = st.columns(2)
        with c_txt:
            st.download_button("📄 Save as Text File", txt_content, "Technodel_Quote.txt", "text/plain", use_container_width=True)
